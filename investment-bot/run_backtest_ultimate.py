...
STOP_LOSS_PCT = -0.15  # 損切り: -15% (非常に緩い、日足ノイズ耐性向上)
...
def run_yearly_backtest(symbol, full_data):
    ...
    for year in years:
        ...
        # 計算値のNaNを埋める
        df_year.fillna(method='bfill', inplace=True)
        ...
        for date, row in df_year.iterrows():
            ...
            if position is None:
                ...
                if row['SMA_Fast'] > row['SMA_Slow'] and current_price > row['SMA_Fast']:
                    ...
                    logging.info(f"    [BUY] {date} | 価格: {current_price:.0f} | 数量: {quantity:.4f} | 資産: {invest_amount:.0f} | 残高: {capital:.0f}")
...
if __name__ == "__main__":
    ...
    logging.info("ライブラリ依存なし (Pandas自実装)")
    logging.info("================================================================================")
