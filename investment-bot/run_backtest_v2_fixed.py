...
STOP_LOSS = -0.20      # 損失設定: -20% (非常に緩い、日足のノイズに絶対に耐える)
BUY_THRESHOLD = 4.0
SELL_THRESHOLD = -4.0
...
def calculate_indicators(df):
    ...
    # ボリンジャーバンド
    bb_indicator = ta.volatility.BollingerBands(close=df['Close'], window=BOLLINGER_PERIOD, std_dev=BOLLINGER_STD)
    bb_df = bb_indicator.bollinger_bands()
    df['Upper_Band'] = bb_df[f'BBU_{BOLLINGER_PERIOD}_{BOLLINGER_STD}']
    df['Middle_Band'] = bb_df[f'BBM_{BOLLINGER_PERIOD}_{BOLLINGER_STD}']
    df['Lower_Band'] = bb_df[f'BBL_{BOLLINGER_PERIOD}_{BOLLINGER_STD}']
    ...
    for date, row in df.iterrows():
        ...
        # ポジションを持っている場合
            ...
            if row['Low'] <= position['stop_loss']:
                ...
                logging.info(f"  取引クローズ: {date} | 理由: stop_loss | エントリー: {position['entry_price']:.2f} | エグジット: {exit_price:.2f} | 変動: {change_pct*100:.2f}% | 利益: {profit:.2f} | 残高: {capital:.2f}")
                position = None
            ...
            # 利益確定のチェック
                ...
                logging.info(f"  取引クローズ: {date} | 理由: take_profit | エントリー: {position['entry_price']:.2f} | エグジット: {exit_price:.2f} | 変動: {change_pct*100:.2f}% | 利益: {profit:.2f} | 残高: {capital:.2f}")
                position = None
...
if __name__ == "__main__":
    ...
    for symbol, df in data.items():
        ...
        # バックテスト実行
        strategy_name = f"2025_fixed" # 修正版
