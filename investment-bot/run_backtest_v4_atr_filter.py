...
# --- 戦略パラメータ (V4: Guardian + ATR Filter) ---
    ...
    def run_yearly_test(self, symbol, full_data):
        ...
        for year in years:
            ...
            for date, row in df_year.iterrows():
                ...
                # --- エントリーロジック (V4: 安全 + ATRフィルター) ---
                if position is None:
                    # 条件1: ゴールデンクロス (SMA50 > SMA200)
                    # 条件2: 価格が長期SMA(200)より上にあること (暴落相場回避)
                    # 条件3 (V4追加): ボラティリティ拡大確認 (ATR > ATR_Mean * 1.5)
                    ...
                    cond_volatility_surge = pd.notna(row['ATR']) and pd.notna(row['ATR_Mean']) and (row['ATR'] > row['ATR_Mean'] * ATR_MULTIPLIER)
...
if __name__ == "__main__":
    logging.info("================================================================================")
    logging.info("V4 Guardian + ATR Filter: 安全性と収益性の両立")
    logging.info("================================================================================")
    ...
    for symbol, df in all_data.items():
        logging.info(f"シンボル: {symbol} のテスト開始 (V4)")
    ...
    logging.info("")
    logging.info("================================================================================")
    logging.info("【最終レポート V4】")
    logging.info("================================================================================")
    
    if global_best['symbol']:
        logging.info("")
        logging.info("  ★★★★ V4 最適解: ATRフィルターによるレンジ相場回避とトレンド追随 ★★★★")
        logging.info("")
        logging.info(f"  最強シンボル: {global_best['symbol']}")
        logging.info(f"  対象年次  : {global_best['year']}年")
        logging.info(f"  達成年利    : {global_best['return']*100:.2f}%")
        logging.info(f"  最終資金 : ¥{int(global_best['capital'])}")
        logging.info("")
        logging.info("  V4 改善点:")
        logging.info("    1. ATRフィルター導入: レンジ相場（低ボラティリティ）でのエントリーを完全阻止")
        logging.info("    2. 暴落相場防御: Price > SMA200 条件でベアー・トラップを回避")
        logging.info("    3. トレイリングストップ: 高値圏での利益確保と急落回避")
        logging.info("")
    else:
        logging.info("  条件達成なし")
